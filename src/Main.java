...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < totalMods; i++) {
            ...
            NT = input.nextInt();
            HashMap<String,Integer> symbol_size=new HashMap<String,Integer>();
            for (int k = 0; k < NT; k++) {
                s = input.next();
                num = input.nextInt();
            ...
            for(String key:symbol_size.keySet()){
                if ( symbol_size.get(key)>= NT) {
                    defList.put(s, mod_address);
                    System.out.println("The definition of " + s + " is outside of module " + i + " . Zero (relative) used.");
                }
            }
            mod_address += NT;
        }// end of first pass
